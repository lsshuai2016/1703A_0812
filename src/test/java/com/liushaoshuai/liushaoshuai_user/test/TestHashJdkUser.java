...
public class TestHashJdkUser {
	...
	public void testJson() throws ParseException {
		...
		for(int i = 0 ; i < 100000 ; i++) {
			users.put("u_"+i,new User(i, StringUtil.generateChineseName()+StringUtil.randomChineseString(2), StringUtil.randomSex(), "13"+RandomUitl.randomString(9), RandomUitl.randomEmail(), DateUtil.randomDate(new SimpleDateFormat("yyyy-MM-dd").parse("1949-01-01")).toString()));
		}
		
		long startTimes = System.currentTimeMillis();
		
			redisTemplate.opsForHash().putAll("users", users);
